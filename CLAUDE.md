...
| **Admin**        | `/apps/admin`        | Vite + React-Admin | Managing forms, texts, configurations | Collapse commentComment on lines R9 to R13Copilot commented on Feb 11, 2026 CopilotAIon Feb 11, 2026ContributorMore actionsThe markdown tables are started with |     | (double pipe), which typically renders as an extra empty column or breaks table formatting depending on the renderer. Use a single leading/trailing pipe for each row (e.g., | App | Path | ... | ) and apply the same fix to all tables in this document.Copilot uses AI. Check for mistakes.Positive FeedbackNegative FeedbackReactWrite a replyResolve comment |
...
}Collapse commentComment on lines R306 to R309Copilot commented on Feb 11, 2026 CopilotAIon Feb 11, 2026ContributorMore actionsDocumenting token usage via a ?token= query param is risky because tokens can leak via logs, analytics, browser history, and the Referer header. If the system must keep this behavior, add an explicit warning and mitigation guidance (e.g., avoid logging full URLs, set a strict Referrer-Policy, and prefer cookies/Authorization headers for new endpoints).Copilot uses AI. Check for mistakes.Positive FeedbackNegative FeedbackReactWrite a replyResolve comment
...
import { handleApiError, hasValidationErrors, isApiErrorArray } from 'handleApiError'Collapse commentComment on line R397Copilot commented on Feb 11, 2026 CopilotAIon Feb 11, 2026ContributorMore actionsThe example import from 'handleApiError' looks like a package import, but later you reference a concrete file path (apps/melding-form/src/handleApiError.ts). To prevent agents from copying a non-resolvable import, update the example to use the actual import path/alias used in this repo (e.g., a relative import or the project’s TS path alias).
...
### JavaScript Code Style PrinciplesCollapse commentComment on line R446Copilot commented on Feb 11, 2026 CopilotAIon Feb 11, 2026ContributorMore actionsThis repo appears to be primarily TypeScript (examples use tsx/typescript). Consider renaming the heading to something like “TypeScript/JavaScript Code Style Principles” to better match the codebase and avoid ambiguity for agents.
