...
describe('getApiErrorMessage', () => {
  it('returns the detail string for a simple API error', () => {
    expect(getApiErrorMessage({ detail: 'Not found' })).toBe('Not found')
  })
  ...
  it('returns joined messages for an API error array with multiple items', () => {
    ...
    expect(getApiErrorMessage(error)).toBe('Field required, Invalid value')
  })
  ...
  it('returns the fallback message for an unknown object', () => {
    expect(getApiErrorMessage({ message: 'something' })).toBe('An unknown error occurred')
  })
  ...
  it('returns the fallback message for a non-object', () => {
    expect(getApiErrorMessage('unexpected string')).toBe('An unknown error occurred')
  })
